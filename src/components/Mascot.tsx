...
// % dari lebar kontainer (bukan dari elemen kilau itu sendiri, makanya
...
export default function Mascot({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      raf = requestAnimationFrame(() => {
        ...
        // Jarak dijinakkan (softened) supaya lirikan tetap halus walau kursor jauh sekali.
        const softened = Math.min(dist / 500, 1);
        const maxOffsetPx = (rect.width * MAX_GAZE_OFFSET_PCT) / 100;
        setGazePx({
          x: (dx / dist) * softened * maxOffsetPx,
          y: (dy / dist) * softened * maxOffsetPx,
        });
      });
    }
  ...
  return (
    <div
      ...
      className={`relative cursor-pointer ${reacting ? "mascot-wiggle" : ""} ${className}`}
    >
      <Image
